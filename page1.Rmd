...
# Comparing Liver Transplants by Donor Types
...
pt_status_df = 
  ...
  # This creates the additional date variables
      mutate(
...
# This code chunk creates a column chart for the frequencies of transplants from living vs deceased donors 
...
# This code chunk creates a frequency table for the frequencies of transplants from living vs deceased donors 
...
```
In the ALD cohort there is a prominent difference between the number of transplants from living donors and the number of transplants from deceased donors. The following analyses will contain a much greater sample of patients who received a transplant from a deceased donor than those who received a transplant from a living donor.

```{r message = FALSE}
# This creates a density plot of recipient age distribution by donor type
...
```
This figure demonstrates the age distribution of recipients based on their transplant donor type. There is a similar trend in the densities of recipient age. However, the recipients with living donors have a sightly greater density among older ages than those with deceased donors. Across both donor types, the greatest density of recipient ages occur approximately between 50-65 years old.


## Patient Status at Last Follow-Up
The patient status at the last follow up (`tfl_lastatus`) was also included to identify if the patient was living, dead, retransplanted, lost to follow up, or not seen.

### Transplants from Deceased Donors
```{r message = FALSE}
# This plots the patient status at the last follow-up for recipients with living donors
...
pt_status_df|>
  ...
  geom_col(position = "dodge")+
    labs(
    title = "Recipient Patient Status at Last Follow-Up (Deceased Donors)",
    x = "Patient Status at Last Follow-Up",
    y = "Frequency",
    fill = "Patient Status"
  )+
...
pt_status_df|>
  ...
  geom_col(position = "dodge")+
      labs(
    title = "Recipient Patient Status at Last Follow-Up (Living Donors)",
    x = "Patient Status at Last Follow-Up",
    y = "Frequency",
    fill = "Patient Status"
  )+
...
There is a similar trend in the frequencies of each category of patient status at the last follow-up. Across donor types, the most frequent follow-up status was "Living", followed by "Dead". The least frequent follow-up status was "Lost to Follow Up", this suggests that there is a strong follow-up retention across the ALD cohort. 
...
# This creates a frequency table of graft failure by donor type 
graft_failures|>
  group_by(don_ty)|>
...
```
From the entire ALD cohort, there were 389 recipients who ever experienced a graft failure. There were 380 graft failures among recipients who received a transplant from a deceased donor and 9 graft failures among recipients who received a transplant from a living donor.


### Time to Graft Failure
```{r message = FALSE}
# This creates a boxplot of Time to Failure
graft_failures|>
  group_by(pers_id)|>
  ggplot()+
...
### Multi-Organ vs Liver Only Transplants

This observed the transplant types in the graft failure subsample. The transplant types were categorized as multi-organ and liver only. 
```{r message = FALSE}
# Frequency table of graft failures by transplant type and donor type
graft_failures|>
  ...
  kable(
    format = "html",
    caption = "Frequency of Graft Failures by Transplant Type and Donor Type",
    align = c("l", "r", "r")
  ) |>
  kable_styling(
    bootstrap_options = c("striped", "hover"),
...
# Plots column chart frequency of graft failures by transplant type and donor type
graft_failures|>
