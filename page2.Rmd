...
tx_data_slim <- liver_data %>%
  group_by(PERS_ID) %>%
  arrange(REC_TX_DT) %>% #lists them chronologically
  summarise(
    first_tx_date = first(REC_TX_DT),
    tfl_death_date = first(TFL_DEATH_DT),
    .groups = 'drop'
  ) %>% # now we have one row per person containing earliest transplant and death after transplant
  mutate(
    first_tx_date = as.Date(first_tx_date), # converting to proper dates
...
waitlist_data <- cand_liin %>%
  ...
  mutate(
    ...
    can_death_date = as.Date(CAN_DEATH_DT), #death while still on the waitlist
    
    # Combine death dates (use most complete)
    death_date = coalesce(can_death_date, tfl_death_date) #picks the first non-missing value to ensure capturing mortality across both datasets
  )
```
...
is_alcohol_related <- function(can_dgn, can_dgn2) {
  ...
  any(c(can_dgn, can_dgn2) %in% alcohol_codes, na.rm = TRUE) #returns true if at least one matches
}

#Build the waitlist cohort
waitlist_cohort <- waitlist_data %>%
  filter( #remove. those with missing identifiers, listing date, or listings before 2020 and <18 years
    !is.na(PERS_ID),
    !is.na(listing_date),
    listing_year >= 2020,
  ...
  mutate(
    ...
    meld_at_listing = as.integer(substr(CAN_INIT_SRTR_LAB_MELD, 3, 4)), #it's formated as e.g. "MELD-24" so we had to extract only the correct numbers
    meld_clean = ifelse(meld_at_listing >= 6 & meld_at_listing <= 40, meld_at_listing, NA), #meld only ranges from 6-40 so the other ones must have been errors
