...
cat("Merged dataset:", nrow(waitlist_data), "candidate records\n")
cat("Variables kept:", ncol(waitlist_data), "\n")
cat("Death dates available:", sum(!is.na(waitlist_data$death_date)), "\n")
cat("Transplant dates available:", sum(!is.na(waitlist_data$first_tx_date)), "\n")
```
...
cat("Waitlist cohort with ALD (2020+):", nrow(waitlist_cohort), "patients\n")
```

```{r create-cohort, echo=FALSE, warning=FALSE, message=FALSE}
...
cat("\n=== OUTCOMES SUMMARY ===\n")
cat("Final cohort:", nrow(waitlist_survival), "patients\n")
cat("Waitlist deaths:", sum(waitlist_survival$waitlist_death, na.rm = TRUE), "\n")
cat("Death rate:", round(mean(waitlist_survival$waitlist_death, na.rm = TRUE) * 100, 1), "%\n\n")
...
table(waitlist_survival$alcohol_subtype)
cat("\n")
table(waitlist_survival$alcohol_detailed)
```
```{r,echo=FALSE, warning=FALSE, message=FALSE}
cat("\n=== BASELINE CHARACTERISTICS BY DIAGNOSIS SUBTYPE ===\n\n")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Create Table stratified by sex (overall)
...
cat("\n=== ALCOHOL-RELATED CIRRHOSIS: BY SEX ===\n")
...
cat("\n=== ACUTE ALCOHOL-RELATED HEPATITIS: BY SEX ===\n")
...
if(nrow(cirrhosis_data) > 0 && sum(cirrhosis_data$waitlist_death) >= 5) {
  ...
  cat("\n=== Alcohol-related Cirrhosis Survival Summary ===\n")
  cat("Total N:", nrow(cirrhosis_data), "\n")
  cat("Deaths:", sum(cirrhosis_data$waitlist_death), "\n")
  cat("Male deaths:", sum(cirrhosis_data$waitlist_death[cirrhosis_data$sex == "Male"]), "\n")
  cat("Female deaths:", sum(cirrhosis_data$waitlist_death[cirrhosis_data$sex == "Female"]), "\n")
}
```
...
if(nrow(aah_data) > 0 && sum(aah_data$waitlist_death) >= 5) {
  ...
  cat("\n=== Acute Alcohol-related Hepatitis Survival Summary ===\n")
  cat("Total N:", nrow(aah_data), "\n")
  cat("Deaths:", sum(aah_data$waitlist_death), "\n")
  cat("Male deaths:", sum(aah_data$waitlist_death[aah_data$sex == "Male"]), "\n")
  cat("Female deaths:", sum(aah_data$waitlist_death[aah_data$sex == "Female"]), "\n")
} else {
  cat("\n=== Acute Alcohol-related Hepatitis ===\n")
  cat("Insufficient events for survival analysis (N =", nrow(aah_data), 
      ", Deaths =", sum(aah_data$waitlist_death), ")\n")
}
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
cat("\n=== COX REGRESSION: ALCOHOL-RELATED CIRRHOSIS ===\n\n")
...
if(nrow(cirrhosis_data) > 0 && sum(cirrhosis_data$waitlist_death) >= 10) {
  ...
  cat(sprintf("\n**Alcohol-related Cirrhosis - Female vs Male:**\n"))
  cat(sprintf("  Adjusted HR: %.2f (95%% CI: %.2f-%.2f, p=%.4f)\n",
              hr_sex, ci_lower, ci_upper, coef_sex["Pr(>|z|)"]))
} else {
  cat("Insufficient events for Cox regression\n")
}

cat("\n\n=== COX REGRESSION: ACUTE ALCOHOL-related HEPATITIS ===\n\n")
...
if(nrow(aah_data) > 0 && sum(aah_data$waitlist_death) >= 10) {
  ...
  cat(sprintf("\n**Acute Alcohol-related Hepatitis - Female vs Male:**\n"))
  cat(sprintf("  Adjusted HR: %.2f (95%% CI: %.2f-%.2f, p=%.4f)\n",
              hr_sex_aah, ci_lower_aah, ci_upper_aah, coef_sex_aah["Pr(>|z|)"]))
} else {
  cat("Insufficient events for Cox regression\n")
  cat("N =", nrow(aah_data), ", Deaths =", sum(aah_data$waitlist_death), "\n")
}
```
